 ...
 * Without `as-child`, acts as the action button itself.
 *
...
export class AlertDialogAction extends LitElement {
  ...
  disconnectedCallback() {
    ...
    if (this.asChild) {
      this._cleanupChildAction();
    } else {
      this._cleanupSelfAsAction();
    }
  }

  private _setupSelfAsAction() {
  ...
  private _cleanupSelfAsAction() {
    this.removeEventListener("click", this._handleClick);
    this.removeEventListener("keydown", this._handleKeyDown);
  }

  private _setupChildAction(child: HTMLElement) {
